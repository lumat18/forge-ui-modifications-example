      ...
      {isCreateContextModalOpen && (
        ...
        >
          <ModalHeader>
            ...
            <p>
              For UI modification {currentUiModification?.name} ({currentUiModification?.id})
            </p>
          </ModalHeader>
          <ModalBody>
            <Form
              onSubmit={(data) => {
                ...
                createUimContext(currentUiModification, data, setContextResult).then(() => {
                  setSubmitDisabled(false);
                  closeCreateContextModal();
                });
              }}
            >
              {({ formProps }) => (
                <form {...formProps}>
                  <ContextSelector setSubmitDisabled={setSubmitDisabled} />

                  <FormFooter>
                    <Button isDisabled={isSubmitDisabled} type="submit" appearance="primary">
                      Submit
                    </Button>
                  </FormFooter>
                </form>
